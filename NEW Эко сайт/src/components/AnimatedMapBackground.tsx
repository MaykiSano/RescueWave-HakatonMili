...
export function AnimatedMapBackground() {
  // Основные города с координатами
  const cities = [
    { name: 'Москва', x: 55, y: 38 },
    { name: 'Токио', x: 75, y: 42 },
    { name: 'Нью-Йорк', x: 25, y: 35 },
    { name: 'Лондон', x: 48, y: 32 },
    { name: 'Сидней', x: 80, y: 75 },
  ];

  const [locations, setLocations] = useState<Location[]>(cities.map(city => ({ ...city, active: Math.random() > 0.5 })));
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      {/* ��инии связи между активными точками */}
